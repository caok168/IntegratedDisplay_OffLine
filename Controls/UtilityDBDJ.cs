namespace UseCase
{
    ...
    public class UtilityDBDJ : OracleDataBaseDJ
    {
    ...
    intRecordID + "," + intTaskID + "," + intPlanID + "," +
        ...
        public void OperatorStore(OracleCommand cmd, long intImpID,long intRecordID, DataTable dtToBegin, int intFlag)
        {
            try
            {
                ...
                else
                {
                    ...
                    UtilityDBDJ.CreateErrorLog("执行各种情况为空!");
                    UtilityDBDJ.CreateErrorLog("执行各种情况为空!");
                    this.strOra = "update tbl02_06pw_tqi t set t.valid=0 where t.standard is null and t.batchimpseq=" + intImpID;
                    cmd.CommandText = this.strOra;
                    try
                    {
                        cmd.ExecuteNonQuery();
                    ...
                    this.strOra = "call wxjc_dev.STATIC_TKY(" + intImpID + ")";
                    cmd.CommandText = this.strOra;
                    try
                    {
                        cmd.ExecuteNonQuery();
        ...
        public DataSet GetDictionaryForAttchmentType()
        {
            ...
            try
            {
                ds= MyComponets.DBUtility.OracleDbHelperDJ.Query(this.strOra);
            }
            catch
            {
