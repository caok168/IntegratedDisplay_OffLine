...
{
    public partial class MultiWaveForm : Form
    {
        CITDataProcess cdp;
        ...
        public void BackGroundRun()
        {
            if (isReady)
            {
                //dataStrList = PreproceingContinousMultiWave(CommonClass.listDIC[0].sFilePath);

                dataStrList = SubProcess(CommonClass.listDIC[0].sFilePath, channelNameEn, channelNameCh, float.Parse(tb_fengzhi.Text), float.Parse(tb_geshu.Text), filter_Fs, filter_Frep_L, filter_Frep_H);
            }
        ...
        private void InitTextboxList()
        {
            ...
            textboxList.Add(textBox_CB_Vt_Geshu);
            textboxList.Add(textBox_CB_Lt_Fengzhi);
            textboxList.Add(textBox_CB_Lt_Geshu);

            textboxList.Add(textBox_Fr_Vt_Fengzhi);
            textboxList.Add(textBox_Fr_Vt_Geshu);
            textboxList.Add(textBox_Fr_Lt_Fengzhi);
            textboxList.Add(textBox_Fr_Lt_Geshu);
        }
        ...
        private Boolean InputCheck()
        {
            ...
            try
            {
                ...
                geshu_CB_Vt = float.Parse(textBox_CB_Vt_Geshu.Text);
                geshu_CB_Lt = float.Parse(textBox_CB_Lt_Geshu.Text);
                geshu_Fr_Vt = float.Parse(textBox_Fr_Vt_Geshu.Text);
                geshu_Fr_Lt = float.Parse(textBox_Fr_Lt_Geshu.Text);
            }
            catch (System.Exception ex)
            {
                MessageBox.Show(ex.Message);
